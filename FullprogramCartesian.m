function  [Matrix_error, Matrix_conf, V12, Xerr_cul] = FullprogramCartesian(Xstart, Xend, Tf, N, method,dt,...
...
% This function is based on CartesianTrajectory in MR library.
timegap = Tf / (N - 1);
...
for i = 1: N
    ...
    % calculate Xd and next timestep end-effector reference configuration
    % Xd_next corresponding based on the Cartesian motion.
    Xd = [Rstart * MatrixExp3(MatrixLog3(Rstart' * Rend) * s1), ...
       pstart + s1 * (pend - pstart); 0, 0, 0, 1];
    Xd_next = [Rstart * MatrixExp3(MatrixLog3(Rstart' * Rend) * s2), ...
       pstart + s2 * (pend - pstart); 0, 0, 0, 1];
    ...
    % calculation of wheel and arm joint speeds V9 (u, thetadot)
