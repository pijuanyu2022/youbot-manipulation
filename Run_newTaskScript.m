...
Tsc_initial = [[1,0,0,1.2];[0,1,0,0];[0,0,1,0.025];[0,0,0,1]];
Tsc_goal = [[0,1,0,0];[-1,0,0,-0.8];[0,0,1,0.025];[0,0,0,1]];
...
Tce_grasp = [[-0.7071,0,0.7071, 1.2];[0,1,0,0];[-0.7071,0,-0.7071,0.0258];[0,0,0,1]]; 
% Assume the height of standoff is 0.3m.
% Then we input the end-effector's standoff configurations above the cube.
Tce_standoff1 = [[-0.7071,0,0.7071,1.2];[0,1,0,0];[-0.7071,0,-0.7071,0.100];[0,0,0,1]];
% Similarly, we can obtain the desired final end-effector's configuration
% and its standoff configuration.
Tce_standoff2 = [[0,1,0,0];[0.7071,0,-0.7071,-0.8];[-0.7071,0,-0.7071,0.100];[0,0,0,1]];
Tce_goal = [[0,1,0,0];[0.7071,0,-0.7071,-0.8];[-0.7071,0,-0.7071,0.0258];[0,0,0,1]];
...
disp('Generating newTaskScript csv file.')
...
csvwrite('newTaskScript.csv', Matrix_conf)%generate csv file for animation
disp('Writing error plot data.')
csvwrite('Xerr data in newTask result.csv', Matrix_error)%generate csv file of error data
